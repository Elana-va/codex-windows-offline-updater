...
这份记录还原 Codex Windows Offline Updater 的真实形成过程。它不是为了重新发明一个安装器，也不是由 9 月 10 日的一次故障临时拼成，而是把从 7 月开始多次实际使用、反复验证的 Codex 更新方法，整理成其他人也能复现的安全流程。

## 这不是一次性方案

目标电脑长期无法稳定依赖 Microsoft Store，因此后续每逢 Codex 发布值得安装的新版本，我们都要重复同一组工作：查询当前版本、取得完整 MSIX、放到 D 盘、验证签名、处理运行中进程、安装后确认版本。

这套方法至少经历了以下阶段：

| 时间 | 实际用途 | 流程变化 |
|---|---|---|
| 2026-07-07 | 第一次解决无法通过 Microsoft Store 正常更新 | 建立“微软 CDN 完整 MSIX + 签名验证 + Add-AppxPackage”路线 |
| 2026 年 7 月后续 | 跟进 Codex 小版本，确认旧方法可以重复使用 | 开始把备份和安装包转移到 D 盘，避免占满系统盘 |
| 2026-08-01 | 再次更新客户端，并处理 Sol/Luna 模型目录 | 更新流程继续有效，但留下静态 `model_catalog_json` 覆盖 |
| 2026 年 9 月上旬 | 在 GPT-6 发布前后继续检查和更新 Codex | 进一步区分桌面包版本、CLI 版本和账号模型开放状态 |
| 2026-09-10 | 更新到 `26.903.8094.0` 并排查 Astra 缺失 | 改用微软 FE3 自动解析，修复响应头兼容问题，加入旧模型目录诊断 |

因此，仓库公开的不是未经验证的教程，而是一套已经在同一台特殊 Windows 环境中多次执行过、每次都根据真实故障继续收紧的 SOP（Standard Operating Procedure，标准作业流程）。
...
第一次成功之后，这条路线又被用于后续小版本更新。重复使用暴露出的实际需求包括：不能每次重新研究下载入口；安装包和备份不能继续挤占 C 盘；应用正在运行时不能粗暴中断正在执行的 Codex 任务；安装完成后必须读取系统实际注册版本，而不是只相信安装命令没有报错。

这些经验后来分别演变为默认 D 盘输出、可复用产品 ID、延迟注册、独立更新辅助进程和安装后诊断。
...
## 2026-08-01：更新流程继续使用，并固定实验模型目录

8 月 1 日再次使用既有方法更新 Codex。与此同时，为了在 Codex 中使用当时尚未稳定进入默认目录的 Sol 和 Luna，配置文件加入了一个本地模型目录覆盖：
...
经过多次更新验证，并在这次解决模型目录故障后，我们没有继续保留一组散落在旧对话里的机器专用命令，而是把流程拆成四个可复用部分：
